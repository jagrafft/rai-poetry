...
"""
def to_DF(rel, cols=None, df=True):
    return (
        # len(rel["output"]) == 1
        single_key_relation_to_DF(rel, df, cols=cols)
        if len(rel["output"]) == 1
        # len(rel["output"]) > 1
        else multikey_relation_to_DF(rel, df)
    )
...
"""
def multikey_relation_to_DF(rel, df):
    """
    ...
    return (DataFrame(data) if df else data)


"""
Transform a single key relation to a Pandas `DataFrame` or Python `dict`.
"""
def single_key_relation_to_DF(rel, df, columns=None):
    if columns is not None:
        ...
        if len(columns) != len(rel["output"][0]["columns"]):
            ...
            return (DataFrame(d) if df else d)
    else:
